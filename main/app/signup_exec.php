...
if (!$email = filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
    header("Location: sign_up.php?error=入力された値が不正です。");
}
...
if(!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    ...
    if (preg_match('/\A(?=.*[a-z])(?=.*?\d)[a-z\d]{8,30}+\z/i', $_POST['password'])) {
        ...
        header("Location: sign_up.php?error=パスワーを入力してください。");
    } else {
        // エラ〜メッセージをsignup.phpに表示させる
        header("Location: sign_up.php?error=パスワードは半角英数字をそれぞれ1文字以上含んだ8~30文字以内で設定してください。");
